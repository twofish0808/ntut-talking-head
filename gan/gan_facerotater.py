...
def compose_tensor(input):
    ...
    compose = [
        ...
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406,0.406], [0.229, 0.224, 0.225,0.255])
    ]
    transform = transforms.Compose(compose)
...
#Adam為一種梯度下降優化演算法
G_optimizer = torch.optim.Adam(model2.parameters(), lr=0.0001, betas = (0.9,0.999))
D_optimizer = torch.optim.Adam(D_model.parameters(), lr=0.000003, betas = (0.5,0.999))
...
img_dataloader = DataLoader(dataset, batch_size=15, shuffle=True)
...
while True:
    ...
    for epoch in range(int(start),n_epoch):
        ...
        for data, target, label2 in img_dataloader:
            ...
            loss=(pix_loss*80+Gan_loss/2)/21
            ...
            print('epoch:[{}], batch:[{}/{}], G_GANC:[{}], G_GANR:[{}], G_GAN[{}], G_full[{}], G_PIXC[{}], G_PIXR[{}], G_PIXA[{}], D_GAN[{}], time:[{}:{}]'.format(epoch, count, len(img_dataloader), round(Gan_loss_c.item(),4),round(Gan_loss_r.item(),4),round(Gan_loss.item(),4),round(loss.item(),4),round(pix_c.item(),4),round(pix_r.item(),4), round(pix_loss.item(),4),round(D_loss.item(),4),int((time.time()-startTime)/60/60), int((time.time()-startTime)/60%60)))
            count+=1
            if (count+1)%50==0:
